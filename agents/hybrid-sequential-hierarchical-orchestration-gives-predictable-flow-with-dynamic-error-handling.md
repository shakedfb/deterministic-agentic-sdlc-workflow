...
The hybrid pattern resolves this by assigning sequential structure to the pipeline level and hierarchical authority to the exception path. In CrewAI terms, this is `Process.sequential` for the crew combined with `allow_delegation=True` on the manager agent. The sequential process ensures that phases execute in the designed order under normal conditions. The hierarchical authority activates when exceptions occur: the orchestrator can override the sequential flow to re-delegate a task to a different specialist, initiate a retry loop bounded by [[hard iteration limits per task are required to prevent runaway loops in multi-agent pipelines]], or escalate to human review, without disrupting the overall pipeline structure. Critically, every re-delegation on the exception path must satisfy the same lossless context requirement as normal handoffs: the context package assembled for the replacement specialist must include all prior iteration history and failure signatures, not just the original task — a requirement established by [[lossless context transfer at handoff boundaries is the orchestrators most critical responsibility]], where incomplete context at exception-path handoffs is shown to be as costly as at normal handoffs.
...
**Relevant Notes:**
- [[lossless context transfer at handoff boundaries is the orchestrators most critical responsibility]] — the exception path's re-delegation decisions require full lossless context assembly, including all prior iteration history; the hybrid pattern's dynamic error handling depends on this principle being honored at exception-path handoffs, not just normal ones
...
- [[hard iteration limits per task are required to prevent runaway loops in multi-agent pipelines]] — the exception path's retry loop is bounded by hard limits; the hierarchical authority that initiates retries also enforces the limit that converts exceeded retries into human escalations
- [[parallel task execution requires dependency graph analysis and is a v2 concern for SDLC pipelines]] — the v2 architecture layer that extends beyond the hybrid pattern; the hybrid sequential-hierarchical design is the v1.5 prerequisite that must be validated before dependency graph analysis and parallel fanout are introduced
