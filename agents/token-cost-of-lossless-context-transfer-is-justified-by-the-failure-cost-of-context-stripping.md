...
- [[hard iteration limits per task are required to prevent runaway loops in multi-agent pipelines]] — the same economic argument extends to escalation-path handoffs: stripping the iteration history from the escalation packet leaves the human reviewer unable to diagnose root cause, producing the same retry-amplified cost that context stripping causes at normal handoffs
- [[what metrics distinguish a well-functioning orchestrator from a coordination bottleneck]] — delegation success rate (healthy threshold: 95%+, warning threshold: 85%) is the measurable expression of this economic argument; the cost of context stripping shows up directly as failed first-attempt delegations, and the 85% warning threshold quantifies the inflection point where stripping has made the orchestrator a serial retry machine
- [[hybrid sequential-hierarchical orchestration gives predictable flow with dynamic error handling]] — the hybrid pattern's exception-path re-delegations carry the same lossless transfer requirement as normal handoffs; the economic argument that makes stripping irrational on the happy path applies with equal force to re-delegation under error conditions, where incomplete context compounds an already-degraded specialist execution
